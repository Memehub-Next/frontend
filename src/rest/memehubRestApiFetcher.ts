import { MemehubRestApiContext } from "./memehubRestApiContext";

const baseUrl = process.env.NEXT_PUBLIC_API_URL;

export type ErrorWrapper<TError> = TError | { status: "unknown"; payload: string };

export type MemehubRestApiFetcherOptions<TBody, THeaders, TQueryParams, TPathParams> = {
  url: string;
  method: string;
  body?: TBody;
  headers?: THeaders;
...
export async function memehubRestApiFetch<
  TData,
  TError,
...
>({
  ...
  queryParams,
  signal,
}: MemehubRestApiFetcherOptions<TBody, THeaders, TQueryParams, TPathParams>): Promise<TData> {
  try {
